...
class DBWriter(object):
    ...
    def __init__(self, user, password, db, host="localhost", sql_dialect="postgres", port="5432"):

        db_connectors = {"postgres": "postgres"}
        self.engine = create_engine(f"{db_connectors[sql_dialect]}://{user}:{password}@{host}:{port}")
        self.create_db_if_not_exist(db, self.engine)
        self.engine = create_engine(f"{db_connectors[sql_dialect]}://{user}:{password}@{host}:{port}/{db}")

    @staticmethod
    def create_db_if_not_exist(db, engine):

        db_list = pd.read_sql("SELECT datname FROM pg_database", engine)["datname"].values
